...
from tobac.inference_ours.tobac import TOBAC 
from tobac.kernel import AdditiveFrobeniusSEKernel, JointAdditiveFrobeniusSEKernel
from tobac.metrics import ParticleDistribution
from tobac.utils.func import expand_by
from tobac.ordering import EqVar, NPVar, SCORE


class MarginalTOBAC(TOBAC):
    """
    This class implements Stein Variational Gradient Descent (SVGD) (Liu and Wang, 2016)
    for TOBAC inference of the marginal DAG posterior :math:`p(G | D)`.
    For joint inference of :math:`p(G, \\Theta | D)`, use the analogous class
    :class:`~tobac.inference.JointTOBAC`.
    ...
    Args:
        ...
        inference_model: Bayes net inference model defining prior :math:`\\log p(G)`
            ...
            Example: :class:`~tobac.models.BGe`
        kernel: Class of kernel. *Has to implement the method* ``eval(u, v)``.
            Example: :class:`~tobac.kernel.AdditiveFrobeniusSEKernel`
    ...
    def __init__(self, *,
        ...
        if perm is None:
            if order_from == "NPVar":
                order = NPVar(x)
            elif order_from == "SCORE":
                order = SCORE(x)
            else:
                order = EqVar(x)
            n_vars = x.shape[-1]
            perm = jnp.vstack([jnp.zeros(n_vars).at[i].set(1) for i in order], dtype=jnp.float32).T
        self.perm = perm
    ...
    def _sample_initial_random_particles(self, *, key, n_particles, n_dim=None):
        ...
        z = random.normal(subk, shape=(n_particles, self.n_vars - 1, n_dim, 2)) * std
    ...
    def _parallel_update_z(self, *args):
        """
        Vectorizes :func:`~tobac.inference.MarginalTOBAC._z_update`
        for all available particles in batched first input dim (``single_z``)
        Otherwise, same inputs as :func:`~tobac.inference.MarginalTOBAC._z_update`.
        """
        return vmap(self._z_update, (0, 1, None, None), 0)(*args)

    def _svgd_step(self, t, opt_state_z, perm, key, sf_baseline):
        """
        Performs a single SVGD step in the TOBAC framework, updating all :math:`Z` particles jointly.
        ...
        # batch_subk = [key] * n_particles
        dz_log_likelihood, sf_baseline = self.eltwise_grad_z_likelihood(z, perm, None, sf_baseline, t, jnp.array(batch_subk))
        # here `None` is a placeholder for theta (in the joint inference case)
        # since this is an inherited function from the general `TOBAC` class

        # d/dz log p(z) (acyclicity)
        key, *batch_subk = random.split(key, n_particles + 1)
        dz_log_prior = self.eltwise_grad_latent_prior(z, perm, jnp.array(batch_subk), t)
        ...
        return opt_state_z, perm, key, sf_baseline
    ...
    def sample(self, *, key, n_particles, steps, n_dim_particles=None, callback=None, callback_every=None):
        """
        Use SVGD with TOBAC to sample ``n_particles`` particles :math:`G` from the marginal posterior
        ...
        for t in (range(0, steps, callback_every) if steps else range(0)):
            ...
            opt_state_z, _, key, sf_baseline = self._svgd_loop(t, callback_every, (opt_state_z, self.perm, key, sf_baseline))
        ...
        g_final = self.particle_to_g_lim(z_final, self.perm)
        return g_final
    ...
    def get_empirical(self, g):
        ...
        Returns:
            :class:`~tobac.metrics.ParticleDistribution`:
...
class JointTOBAC(TOBAC):
    ...
    Args:
        ...
        inference_model: Bayes net inference model defining prior :math:`\\log p(G)`
            ...
            Example: :class:`~tobac.models.LinearGaussian`
        kernel: Class of kernel. *Has to implement the method* ``eval(u, v)``.
            Example: :class:`~tobac.kernel.JointAdditiveFrobeniusSEKernel`
        kernel_param (dict): kwargs to instantiate ``kernel``
        optimizer (str): optimizer identifier
        optimizer_param (dict): kwargs to instantiate ``optimizer``
        alpha_linear (float): slope of of linear schedule for inverse temperature :math:`\\alpha`
    ...
    def _parallel_update_z(self, *args):
        """
        Vectorizes :func:`~tobac.inference.JointTOBAC._z_update`
        for all available particles in batched first and second input
        dim (``single_z``, ``single_theta``)
        Otherwise, same inputs as :func:`~tobac.inference.JointTOBAC._z_update`.
        """
    ...
    def _theta_update(self, single_z, single_theta, kxx, z, theta, grad_log_prob_theta):
        ...
        Analogous to :func:`tobac.inference.JointTOBAC._z_update` but for updating :math:`\Theta`.
    ...
    def _parallel_update_theta(self, *args):
        """
        Vectorizes :func:`~tobac.inference.JointTOBAC._theta_update`
        for all available particles in batched first and second input
        dim (``single_z``, ``single_theta``).
        Otherwise, same inputs as :func:`~tobac.inference.JointTOBAC._theta_update`.
        """
        return vmap(self._theta_update, (0, 0, 1, None, None, None), 0)(*args)


    def _svgd_step(self, t, opt_state_z, perm, opt_state_theta, key, sf_baseline):
        """
        Performs a single SVGD step in the TOBAC framework, updating all :math:`(Z, \\Theta)` particles jointly.
        ...
        key, *batch_subk = random.split(key, n_particles + 1)
        # batch_subk = [key] * n_particles
        dtheta_log_prob = self.eltwise_grad_theta_likelihood(z, perm, theta, t, jnp.array(batch_subk))

        # d/dz log p(theta, D | z)
        key, *batch_subk = random.split(key, n_particles + 1)
        dz_log_likelihood, sf_baseline = self.eltwise_grad_z_likelihood(z, perm, theta, sf_baseline, t, jnp.array(batch_subk))
        ...
        return opt_state_z, perm, opt_state_theta, key, sf_baseline
    ...
    def sample(self, *, key, n_particles, steps, n_dim_particles=None, callback=None, callback_every=None):
        """
        Use SVGD with TOBAC to sample ``n_particles`` particles :math:`(G, \\Theta)` from the joint posterior
        ...
        for t in (range(0, steps, callback_every) if steps else range(0)):
            ...
            # perform sequence of SVGD steps
            opt_state_z, _, opt_state_theta, key, sf_baseline = self._svgd_loop(t, callback_every,
                                                                             (opt_state_z, self.perm, opt_state_theta, key,
                                                                              sf_baseline))
