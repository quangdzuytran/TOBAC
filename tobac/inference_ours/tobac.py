...
from tobac.graph_utils import acyclic_constr_nograd
from tobac.utils.func import expand_by, zero_diagonal, upper

EPS = 10e-16

class TOBAC:
    """
    This class implements the backbone for TOBAC, i.e. all gradient estimators and sampling
    components. Any inference method in the TOBAC framework should inherit from this class.
    ...
    Args:
        ...
        log_graph_prior (callable):
            ...
            edge probabilities.
            For example: :func:`~tobac.graph.ErdosReniDAGDistribution.unnormalized_log_prob_soft`
            or usually bound in e.g. :func:`~tobac.graph.LinearGaussian.log_graph_prior`
        log_joint_prob (callable):
            ...
            For example: :func:`tobac.models.LinearGaussian.observational_log_joint_prob`.
    ...
    def particle_to_g_lim(self, z, perm):
        """
        ...
        su = upper((scores > 0).astype(jnp.int32))
        g_samples = perm @ jnp.pad(su, ((0, 0), (0, 1), (1, 0)), mode='constant', constant_values=0) @ perm.T
    ...
    def particle_to_soft_graph(self, z, perm, eps, t):
        """
        ...
        # eps ~ Logistic(0,1)
        soft_su = upper(sigmoid(self.tau * (eps + self.alpha(t) * scores)))
        soft_graph = perm @ jnp.pad(soft_su, ((0, 1), (1, 0)), mode='constant', constant_values=0) @ perm.T
    ...
    def particle_to_hard_graph(self, z, perm, eps, t):
        """
        ...
        hard_su = upper(((eps + self.alpha(t) * scores) > 0.0).astype(jnp.float32))
        hard_graph = perm @ jnp.pad(hard_su, ((0, 1), (1, 0)), mode='constant', constant_values=0) @ perm.T
    ...
    def edge_probs(self, z, perm, t):
        ...
        u, v = z[..., 0], z[..., 1]
        scores = jnp.einsum('...ik,...jk->...ij', u, v)
        su_probs = upper(sigmoid(self.alpha(t) * scores))
        if len(u.shape) == 2:
            pad = ((0, 1), (1, 0))
        else:
            pad = ((0, 0), (0, 1), (1, 0))
        probs = perm @ jnp.pad(su_probs, pad, mode='constant', constant_values=0) @ perm.T
    ...
    def edge_log_probs(self, z, perm, t):
        """
        ...
        scores = jnp.einsum('...ik,...jk->...ij', u, v)
        su_probs = upper(sigmoid(self.alpha(t) * scores))
        probs = perm @ jnp.pad(su_probs, ((0, 1), (1, 0)), mode='constant', constant_values=0) @ perm.T

        log_probs, log_probs_neg = jnp.log(probs + EPS), jnp.log(1 - probs + EPS)
    ...
    def latent_log_prob(self, single_g, single_z, perm, t):
        """
        ...
        """
        # [d, d], [d, d]
        log_p, log_1_p = self.edge_log_probs(single_z, perm, t)
    ...
    def eltwise_grad_latent_log_prob(self, gs, single_z, perm, t):
        """
        ...
        return vmap(dz_latent_log_prob, (0, None, None, None), 0)(gs, single_z, perm, t)
    ...
    def log_joint_prob_soft(self, single_z, perm, single_theta, eps, t, subk):
        """
        ...
        """
        soft_g_sample = self.particle_to_soft_graph(single_z, perm, eps, t)
    ...
    def eltwise_grad_z_likelihood(self, zs, perm, thetas, baselines, t, subkeys):
        """
        ...
        # vmap
        return vmap(grad_z_likelihood, (0, None, 0, 0, None, 0), (0, 0))(zs, perm, thetas, baselines, t, subkeys)



    def grad_z_likelihood_score_function(self, single_z, perm, single_theta, single_sf_baseline, t, subk):
        """
        ...
        # [d, d]
        p = self.edge_probs(single_z, perm, t)
        ...
        grad_z = self.eltwise_grad_latent_log_prob(g_samples, single_z, perm, t) \
    ...
    def grad_z_likelihood_gumbel(self, single_z, perm, single_theta, single_sf_baseline, t, subk):
        """
        ...
        Examples are: ``tobac.models.LinearGaussian`` and ``tobac.models.DenseNonlinearGaussian``
        ...
        logprobs_numerator = vmap(self.log_joint_prob_soft, (None, None, None, 0, None, None), 0)(single_z, perm, single_theta, eps, t, subk_)
        logprobs_denominator = logprobs_numerator
        ...
        grad_z = vmap(grad(self.log_joint_prob_soft, 0), (None, None, None, 0, None, None), 0)(single_z, perm, single_theta, eps, t, subk_)
    ...
    def eltwise_grad_theta_likelihood(self, zs, perm, thetas, t, subkeys):
        """
        ...
        """
        return vmap(self.grad_theta_likelihood, (0, None, 0, None, 0), 0)(zs, perm, thetas, t, subkeys)


    def grad_theta_likelihood(self, single_z, perm, single_theta, t, subk):
        """
    ...
    """
    def constraint_gumbel(self, single_z, perm, single_eps, t):
        """
        ...
        """
        n_vars = single_z.shape[0]
        G = self.particle_to_soft_graph(single_z, perm, single_eps, t)
        h = acyclic_constr_nograd(G, n_vars)
        return h


    def grad_constraint_gumbel(self, single_z, perm, key, t):
        """
        ...
        # [n_mc_samples, d, k, 2]
        mc_gradient_samples = vmap(grad(self.constraint_gumbel, 0), (None, None, 0, None), 0)(single_z, perm, eps, t)

        # [d, k, 2]
        return mc_gradient_samples.mean(0)


    def log_graph_prior_particle(self, single_z, perm, t):
        """
        ...
        single_soft_g = self.edge_probs(single_z, perm, t)

        # [1, ]
        return self.log_graph_prior(single_soft_g)


    def eltwise_grad_latent_prior(self, zs, perm, subkeys, t):
        """
        Computes batch of estimators for the score :math:`\\nabla_Z \\log p(Z)`
        with
        ...
        computed inside ``tobac.inference.TOBAC.log_graph_prior_particle``.
        ...
        grad_prior_z = vmap(grad_log_graph_prior_particle, (0, None, None), 0)(zs, perm, t)

        # constraint term
        # [n_particles, d, k, 2], [n_particles,], [1,] -> [n_particles, d, k, 2]
        # eltwise_grad_constraint = vmap(self.grad_constraint_gumbel, (0, None, 0, None), 0)(zs, perm, subkeys, t)

        # return - self.beta(t) * eltwise_grad_constraint \
        #        - zs / (self.latent_prior_std ** 2.0) \
        #        + grad_prior_z
        return - zs / (self.latent_prior_std ** 2.0) \
               + grad_prior_z
    ...
    def visualize_callback(self, ipython=True, save_path=None):
        ...
        from tobac.utils.visualize import visualize
        from tobac.graph_utils import elwise_acyclic_constr_nograd as constraint
        if ipython:
        ...
        def callback(**kwargs):
            zs = kwargs["zs"]
            perm = kwargs["perm"]
            gs = kwargs["dibs"].particle_to_g_lim(zs, perm)
            probs = kwargs["dibs"].edge_probs(zs, perm, kwargs["t"])
            if ipython:
